      ...
      <Box sx={{ background: COLORS.background, minHeight: '100vh', mx: -3, mt: -2, px: 3, py: 2 }}>
        ...
        <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2}>
          ...
          >
            ...
            <Box sx={{ p: 2, borderBottom: `1px solid ${COLORS.border}`, background: `linear-gradient(135deg, ${COLORS.primary}10, ${COLORS.primaryLight}05)` }}>
              ...
              <Typography variant="caption" color={COLORS.textSecondary}>
                Tự động cập nhật khi thêm/hủy món
              </Typography>
            </Box>
            ...
            <Box sx={{ p: 2, background: `linear-gradient(135deg, ${COLORS.primary}08, ${COLORS.primaryLight}05)` }}>
              <Stack spacing={2}>
                <TextField
                  size="small"
                  placeholder="Tìm theo bàn, mã HĐ..."
                  value={searchQuery}
              ...
              {!isLoading && filteredInvoices.length === 0 && (
                <Box sx={{ textAlign: 'center', py: 6 }}>
                  ...
                  <Typography color={COLORS.textSecondary}>
                    {searchQuery ? 'Không tìm thấy hóa đơn' : 'Chưa có bàn nào đang phục vụ'}
                  </Typography>
                  <Typography variant="caption" color={COLORS.textMuted} sx={{ mt: 1, display: 'block' }}>
                    Hóa đơn sẽ tự động xuất hiện khi tạo order mới
                  </Typography>
                </Box>
              )}
            </Box>
                          ...
                          <Box component="li" {...props} key={option.id}>
                            <Stack>
                              ...
                              <Typography variant="caption" color="text.secondary">
                                📞 {option.soDienThoai} • ⭐ {option.diemTichLuy || 0} điểm
                              </Typography>
                            </Stack>
                          </Box>
                        )}
        ...
        >
          ...
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setNewCustomerDialog(false)}>Hủy</Button>
            <Button
              variant="contained"
            ...
            >
              {createCustomer.isLoading ? 'Đang thêm...' : 'Thêm khách hàng'}
            </Button>
          </DialogActions>
        </Dialog>
